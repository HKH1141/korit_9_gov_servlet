...
/*
HTTP 프로토콜 Method
Get-용도: 리소스 조회-특징: 서버로부터 데이터를 요청만 하고 수정하지 않음
       요청 데이터(파라미터)가 URL에 포함됨
       브라우저 히스토리에 남음 (뒤로가기 사용가능한 이유)
        북마크 가능
       캐싱 가능

POST-용도: 새로운 리소스 생성-특징:
     서버에 데이터를 전송하여 새로운 리소스 생성
     요청 데이터가 HTTP Body에 포함됨
     브라우저 히스토리에 남지 않음
     캐싱되지 않음

PUT-용도: 리소스 전체 수정/ 생성-특징:
     리소스가 있으면 전체를 교체, 없으면 생성
     전체 데이터를 전송해야함
     멱등성 있음

Patch-용도: 리소스 부분 수정-특징:
     리소스의 일부만 수정
     Put 보다 효율적 (변경할 필드만 전송)
     멱등성 있음
...
HEAD-용도: 리소스 존재여부 또는 메타데이터 확인

OPTION-용도: HTTP 메서드의 존재여부 또는 CORS 프리플라이트 요청에 사용

CONNECT-용도: 프록시 서버를 통한 터널링에 사용, SSL 연결에 활용됨
TRACE-용도: 디버깅, 요청 경로 루프백 테스트
